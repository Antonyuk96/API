import { accessKey } from '/API/3/accessKey.js';
...
// Ввести ключ доступа
const accessKeyIm = accessKey;

quantity.addEventListener("change", () => {
   loadMorePhotos(quantity.value);
});

async function fetchPhoto (numberPage, quantity) {
   try {
      const response = await fetch(`https://api.unsplash.com/photos?page=${numberPage}&per_page=${quantity}&client_id=${accessKeyIm}&lang=ru`);
...
async function loadMorePhotos (quantityPhoto) {
   fetchPhoto(number, quantityPhoto).then((photos) => {
...
// Первые фотографии при загрузке страницы
loadMorePhotos(quantity.value);
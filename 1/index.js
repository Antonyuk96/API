...
containerEl.addEventListener("click", function (e) {
  ...
  // Кнопка записи неактивна при достижении максимального количества участников
  ...
  if (e.target.classList.contains("join")) {
    //Изменение количества записанных участников в html
    ...
    //Если пользователь уже записался-кнопка записи неактивна, кнопка отмены - активна
    joinBtnElems[indexActivity].disabled = true;
    cancelBtnElems[indexActivity].disabled = false;
  }
  if (e.target.classList.contains("cancel")) {
    //Уменьшение количества записанных участников в html
    currentMembersEl.textContent = currentMembers - 1;

    //Изменение количества участников в массиве и запись в localStorage
    activities[indexActivity].currentParticipants -= 1;
    localStorage.setItem(localStorageKey, JSON.stringify(activities));

    // Кнопка записи неактивна, если пользователь уже записался, а кнопка отмены - активна
